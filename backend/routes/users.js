...
router.get("/", authMiddleware, async (_req, res) => {
  try {
    const [users] = await db.query(
      "SELECT id, first_name, last_name, age, gender, profile_photo, email, created_at FROM users ORDER BY last_name ASC"
    );
    res.json(users);
  } catch (err) {
...
router.get("/:id", authMiddleware, async (req, res) => {
  try {
    const [users] = await db.query(
      "SELECT id, first_name, last_name, age, gender, profile_photo, email, created_at FROM users WHERE id = ?",
      [req.params.id]
    );
